...
function App() {
  ...
  // Modules division based on team's architecture
  const coreModules = [
    {
      icon: "💊",
  ...
  return (
    <div className="flex flex-col min-h-screen bg-lightBg selection:bg-primary/20">
      ...
      <main className="flex-grow">
        <Routes>
          {/* 1. HOME ROUTE (App's whole landing screen) */}
          <Route path="/" element={
            <>
              {/* Hero Section */}
              <section className="relative overflow-hidden bg-gradient-to-b from-blue-50/50 via-white to-lightBg pt-16 pb-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 animate-fade-in">
                  ...
                  <h1 className="mt-6 text-4xl sm:text-5xl lg:text-6xl font-black text-dark tracking-tight max-w-4xl mx-auto leading-tight">
                    Revolutionizing Healthcare with <span className="text-primary">Smart Digital Care</span>
                  </h1>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center">
                    {stats.map((stat, i) => (
                      <div key={i} className="p-4">
                        ...
                        <div className="mt-2 text-xs sm:text-sm font-semibold text-slate-500 uppercase tracking-wider">{stat.label}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                <div className="text-center mb-16">
                  ...
                  <p className="mt-4 text-slate-500 max-w-xl mx-auto text-sm sm:text-base">
                    The project structure is split into robust functional pipelines. Click on any block to begin testing the implementation.
                  </p>
                </div>
                  ...
                  {coreModules.map((module, i) => (
                    ...
                    >
                      <div>
                        ...
                        <h3 className="text-xl font-bold text-dark mb-3">{module.title}</h3>
              ...
              {/* Feature Showcase / Lab Section */}
              <section className="bg-slate-900 text-white py-16">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                  <div>
                    <span className="text-secondary font-bold uppercase tracking-widest text-xs">Diagnostic Integrity</span>
                    <h2 className="text-3xl sm:text-4xl font-extrabold mt-3 tracking-tight">
                      IntegrDiagnostic Labs Instantly
                    </h2>
                    <p className="mt-4 text-slate-400 text-sm sm:text-base leading-relaxed">
                      Connect your medical diagnostics pipeline seamlessly. Patients can upload test reports, view dynamic tracking states, and receive interactive graphical feedback instantly.
                    </p>
                    <div className="mt-8">
                      <Button variant="secondary" onClick={() => navigate('/lab-tests')}>
                        Manage Lab Testing
                      </Button>
                    </div>
                  </div>
                  <div className="bg-slate-800/80 border border-slate-700 p-8 rounded-3xl relative overflow-hidden">
                    <div className="absolute top-0 right-0 bg-secondary/10 text-secondary text-xs px-3 py-1 rounded-bl-xl font-bold">LIVE METRIC</div>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center border-b border-slate-700 pb-3">
                        <span className="text-sm font-medium text-slate-300">Blood Analysis Setup</span>
                        <span className="text-xs font-semibold px-2 py-1 rounded bg-teal-500/10 text-secondary">Active</span>
                      </div>
                      <div className="flex justify-between items-center border-b border-slate-700 pb-3">
                        <span className="text-sm font-medium text-slate-300">ECG Processing Pipeline</span>
                        <span className="text-xs font-semibold px-2 py-1 rounded bg-teal-500/10 text-secondary">Ready</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-slate-300">AI Symptom Check Agent</span>
                        <span className="text-xs font-semibold px-2 py-1 rounded bg-blue-500/10 text-primary">Integrating</span>
                      </div>
                    </div>
                  </div>
                </div>
